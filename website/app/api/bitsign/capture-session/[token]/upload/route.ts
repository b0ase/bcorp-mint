...
/**
 * GET /api/bitsign/capture-session/[token]/upload
 * Validate session (phone checks if token is valid before showing camera).
 * No auth required — the phone only has the session token.
 */
...
/**
 * POST /api/bitsign/capture-session/[token]/upload
 * Upload captured media (phone sends photo/video).
 * No auth required — validated by session token.
 */
