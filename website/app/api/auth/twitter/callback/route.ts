...
export async function GET(request: NextRequest) {
    ...
    if (!code || !stateParam) {
        return NextResponse.redirect(`${APP_URL}/user/account?error=no_code`);
    }
    ...
    try {
        ...
        return NextResponse.redirect(`${APP_URL}/user/account?error=${encodeURIComponent(error.message)}`);
    }
}
