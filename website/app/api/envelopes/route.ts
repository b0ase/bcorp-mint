...
export async function POST(request: NextRequest) {
  try {
    ...
    const signingUrls = enrichedSigners.map((s: any) => ({
      name: s.name,
      role: s.role,
      url: `${process.env.NEXT_PUBLIC_APP_URL || 'https://bitcoin-mint.com'}/sign/${s.signing_token}`,
    }));
          ...
          if (payments.length > 0) {
            try {
              await senderAccount.wallet.pay({
                description: `Bitcoin Mint: Signing fee for "${title}" (${payments.length} signer${payments.length > 1 ? 's' : ''})`,
                payments,
              });
    ...
    if (handCashConnect && process.env.HOUSE_AUTH_TOKEN) {
      ...
      for (const s of enrichedSigners) {
        if (s.handle) {
          try {
            const signingUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'https://bitcoin-mint.com'}/sign/${s.signing_token}`;
            const service = (houseAccount as any).handCashConnectService;
            const requestParams = {
              description: `Bitcoin Mint: ${handle} sent you "${title}" to sign`,
              payments: [{
    ...
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://bitcoin-mint.com';
    for (const s of enrichedSigners) {
      if (s.email) {
        try {
