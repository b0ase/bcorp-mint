...
export default function VaultPage() {
  ...
  const fetchTrash = useCallback(async (userHandle: string) => {
    try {
      const res = await fetch(`/api/bitsign/signatures?handle=${userHandle}&trash=true`);
      if (!res.ok) return;
      const data = await res.json();
  ...
  const handleFileUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        // Upload to vault via inscription API
        const rawBase64 = imageData.replace(/^data:[^;]+;base64,/, '');
        const res = await fetch('/api/bitsign/inscribe', {
          method: 'POST',
  ...
  const deleteItem = useCallback(async (sigId: string) => {
    ...
    setContextMenu(null);
  }, [handle, expandedSig, fetchData, fetchTrash, addToast]);
  ...
  }, [handle, fetchData, fetchTrash, addToast]);
  ...
  const handleSeal = useCallback(async (compositeBase64: string, elements: PlacedElement[]) => {
    ...
    try {
      const res = await fetch('/api/bitsign/seal', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  }, [selectedDocId, handle, fetchData, addToast]);
  ...
  const handleWalletSignComplete = useCallback(async (result: {
    ...
    try {
      const res = await fetch(`/api/bitsign/signatures/${walletModalTarget}/attest`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  }, [walletModalTarget, handle, fetchData, addToast]);
