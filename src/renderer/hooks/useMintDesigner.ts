...
export function useMintDesigner() {
  ...
  // Add or replace the portrait/artwork image layer
  // Inserts after background layers but below all pattern/text layers
  const addImageFromUrl = useCallback((src: string, name: string) => {
    pushUndo(doc);
    ...
    const newLayer: MintLayer = {
      ...
      transform: defaultTransform(),
      type: 'image',
      config: { src, fit: 'cover' as const, x: 0.5, y: 0.5, scale: 1.2 }
    } as MintLayer;

    setDoc((prev) => {
      // Remove any existing image layers and portrait overlays (swap, don't stack)
      const withoutImages = prev.layers.filter(l => l.type !== 'image' && l.name !== 'Portrait Fade' && l.name !== 'Colour Wash');
      // Insert after background gradient (index 1) so patterns show on top
      const insertAt = Math.min(1, withoutImages.length);
      const layers = [...withoutImages];
      // Image layer
      layers.splice(insertAt, 0, newLayer);
      // Overlay gradient above image — fades edges into the note
      const fadeLayer: MintLayer = {
        id: crypto.randomUUID(),
        name: 'Portrait Fade',
        visible: true,
        locked: false,
        ...
        transform: defaultTransform(),
        type: 'gradient',
        config: { type: 'radial', colors: ['rgba(0,0,0,0.1)', prev.backgroundColor || '#000000'], angle: 0, opacity: 0.85 }
      } as MintLayer;
      layers.splice(insertAt + 1, 0, fadeLayer);

      // Additional colour wash — tints the image to match the note palette
      const washLayer: MintLayer = {
        id: crypto.randomUUID(),
        name: 'Colour Wash',
        visible: true,
        locked: false,
        ...
        transform: defaultTransform(),
        type: 'gradient',
        config: { type: 'linear', colors: [prev.backgroundColor || '#000000', 'transparent', prev.backgroundColor || '#000000'], angle: 0, opacity: 0.5 }
      } as MintLayer;
      layers.splice(insertAt + 2, 0, washLayer);
      return { ...prev, layers };
    });
