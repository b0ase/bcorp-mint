...
export function generateQRCode(width: number, height: number, config: QRCodeConfig): string {
  ...
  parts.push(`<rect x="${ox - pad}" y="${oy - pad}" width="${qrSize + pad * 2}" height="${qrSize + pad * 2}" rx="4" fill="${config.backgroundColor !== 'transparent' ? config.backgroundColor : '#000000'}" />`);
  parts.push(`<rect x="${ox - pad}" y="${oy - pad}" width="${qrSize + pad * 2}" height="${qrSize + pad * 2}" rx="4" fill="none" stroke="${config.color}" stroke-width="2" opacity="0.5" />`);
