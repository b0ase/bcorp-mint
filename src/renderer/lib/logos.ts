...
const buildHorizontal = (c: LogoDesignerConfig): string => {
  ...
  const subEl = safeSub
    ? `<text x="50%" y="68%" text-anchor="middle" dominant-baseline="middle" font-family="'Space Grotesk', sans-serif" font-weight="400" font-size="${Math.round(c.fontSize * 0.35)}" letter-spacing="${c.letterSpacing * 0.8}" fill="${sc}" opacity="${c.opacity * 0.7}">${safeSub}</text>`
    : '';
...
const buildStacked = (c: LogoDesignerConfig): string => {
  ...
  const ruleEl = hasRule
    ...
    : '';
  const subEl = safeSub
    ? `<text x="50%" y="65%" text-anchor="middle" dominant-baseline="middle" font-family="'Space Grotesk', sans-serif" font-weight="400" font-size="${Math.round(c.fontSize * 0.4)}" letter-spacing="${c.letterSpacing}" fill="${sc}" opacity="${c.opacity * 0.7}">${safeSub}</text>`
    : '';
...
const buildBadge = (c: LogoDesignerConfig): string => {
  ...
  const subEl = safeSub
    ? `<text x="250" y="350" text-anchor="middle" dominant-baseline="middle" font-family="'Space Grotesk', sans-serif" font-weight="400" font-size="${Math.round(c.fontSize * 0.3)}" letter-spacing="${c.letterSpacing * 0.6}" fill="${sc}" opacity="${c.opacity * 0.7}">${safeSub}</text>`
    : '';
  ...
  <text x="250" y="${safeSub ? '270' : '300'}" text-anchor="middle" dominant-baseline="middle" ${textAttrs(c)}>${safeText}</text>
  ${subEl}
