...
import WebMExporter from './components/WebMExporter';
...
import type { ActiveIssue, AppMode, AudioSegment, ExtractedFrame, ImageItem, ImageSettings, LogoAsset, Spread, StampReceipt, TextOverlay, WalletState } from './lib/types';
import { createDefaultTextOverlay } from './lib/defaults';
...
function buildSpreads(images: ImageItem[]): Spread[] {
  ...
  while (i < images.length) {
    ...
    // Videos and audio always get their own spread — never pair them
    if (isLandscape || img.mediaType === 'video' || img.mediaType === 'audio') {
      spreads.push({ type: 'landscape', image: img });
      i++;
    } else {
      const next = images[i + 1];
      // Only pair two portrait IMAGES — never videos/audio
      if (next && next.mediaType === 'image' && next.height >= next.width) {
...
export default function App() {
  ...
  // Auto-add selected media to Mint canvas when in Mint mode
  // Works for images (direct URL) and videos (thumbnail URL — both are JPEG data URLs)
  useEffect(() => {
    if (tokenisation.mode !== 'mint' || !selectedId) return;
    const img = images.find((i) => i.id === selectedId);
    if (!img || img.mediaType === 'audio' || !img.url) return;
    mint.addImageFromUrl(img.url, img.name);
  }, [tokenisation.mode, selectedId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const videoImages = useMemo(() => sortedImages.filter((img) => img.mediaType === 'video'), [sortedImages]);

  const currentIssue = useMemo(() => issues.find((i) => i.id === currentIssueId) ?? null, [issues, currentIssueId]);
  const enabledImages = useMemo(() => {
    const list = sortedImages.filter((img) => !img.disabled);
    if (!currentIssue) return list;
    return list.filter((img) => currentIssue.enabledIds.has(img.id));
  }, [sortedImages, currentIssue]);
  ...
  // --- Unload / Toggle / Reorder ---

  const handleRemoveImage = useCallback((id: string) => {
    setImages((prev) => prev.filter((img) => img.id !== id));
    // Clean up from issues
    setIssues((prev) => prev.map((issue) => {
      const next = new Set(issue.enabledIds);
      next.delete(id);
      return { ...issue, enabledIds: next };
    }));
    // Clean up tokenisation data
    tokenisation.clearFrames(id);
    setAudioPeaks((prev) => { const m = new Map(prev); m.delete(id); return m; });
    // Reset selection if removing selected
    if (selectedId === id) {
      setSelectedId((prev) => {
  ...
  }, [selectedId, images, tokenisation]);
      ...
      filePaths.map(async (filePath): Promise<ImageItem | null> => {
        try {
          ...
          if (isVideoFile(filePath)) {
            // Use ffmpeg for fast thumbnail — doesn't load entire file into memory
            let width = 1280, height = 720, thumbnailUrl = '';
            try {
              const thumb = await window.mint.extractThumbnail(filePath);
              width = thumb.width;
              height = thumb.height;
              ...
              try {
                ...
                const result = await loadVideoThumbnail(streamUrl);
                width = result.width;
                height = result.height;
                thumbnailUrl = result.thumbnailUrl;
              } catch { /* skip this video */ return null; }
            }
            ...
            // Get real dimensions from probe if available
            if (probe && (probe as any).width) width = (probe as any).width;
            if (probe && (probe as any).height) height = (probe as any).height;
            return {
              id: crypto.randomUUID(),
              path: filePath,
              name,
  ...
  }, [handlePrevSpread, handleNextSpread, tokenisation.mode, mint, selectedId, selectedTextId, selectedImage, handleRemoveImage, handleAddTextOverlay, handleUpdateTextOverlay]);
  ...
  const handleSelectCover = useCallback(async (filePath: string) => {
    await loadFilePaths([filePath]);
    // Set its sortOrder to -1 so it becomes the first image (cover/page 1)
    setImages((prev) => {
      const last = prev[prev.length - 1];
      if (!last) return prev;
      return prev.map((img) =>
        img.id === last.id ? { ...img, sortOrder: -1 } : img
      );
    });
  }, [loadFilePaths]);

  const handleApplyTemplate = useCallback((overlays: import('./lib/types').TextOverlay[]) => {
    // Apply template overlays to the first enabled image (cover)
  ...
  return (
    <div className="app" onDrop={handleDrop} onDragOver={handleDragOver}>
      ...
      ) : tokenisation.mode === 'mix' ? (
        <VideoMixer mediaItems={images} logos={logos} />
      ) : tokenisation.mode === 'ticket' ? (
        <TicketDesigner image={selectedImage || null} logos={logos} />
      ) : tokenisation.mode === 'export' ? (
        <WebMExporter videos={images} selectedVideo={selectedImage || null} onSelectVideo={id => setSelectedId(id)} />
      ) : tokenisation.mode === 'titles' ? (
      ...
      <div className="main">
                  ...
                  >
                    ...
                    <div className="image-card-actions">
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          if (currentIssue) toggleImage(image.id);
                          else handleToggleDisabled(image.id);
                        }}
                        title={currentIssue ? (inIssue ? 'Remove from issue' : 'Add to issue') : (image.disabled ? 'Enable' : 'Disable')}
                      >
                        {currentIssue ? (inIssue ? '\u25C9' : '\u25CB') : (image.disabled ? '\u25CB' : '\u25C9')}
                      </button>
                      <button
          ...
          tokenisation.mode === 'tokenise' && selectedImage?.mediaType === 'video' && currentFrames.length > 0 ? (
            ...
            <>
              <div className="canvas-video-wrap">
                ...
                {/* Play button removed — use canvas-helper bar controls instead */}
                {playingVideo && videoImages.length > 0 && (
                  <VideoSlideshow
                    videos={videoImages}
                    logos={logos}
                    looping={videoLoop}
                    startIndex={selectedImage?.mediaType === 'video' ? Math.max(0, videoImages.findIndex(v => v.id === selectedImage.id)) : 0}
              ...
              <div className="canvas-helper">
                ...
                {/* Video playback controls — shown when videos are available */}
                {videoImages.length > 0 && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <button
                      className={`ghost spread-nav${playingVideo ? ' active' : ''}`}
                      onClick={() => setPlayingVideo(!playingVideo)}
                      title={playingVideo ? 'Stop playback' : 'Play videos'}
                      style={playingVideo ? { color: 'var(--accent)' } : {}}
                    >
                      {playingVideo ? '\u23F9' : '\u25B6'}
                    </button>
                    <button
                      className={`ghost spread-nav${videoLoop ? ' active' : ''}`}
                      onClick={() => setVideoLoop(!videoLoop)}
                      title={videoLoop ? 'Loop ON' : 'Loop OFF'}
                      style={videoLoop ? { color: 'var(--accent)' } : { opacity: 0.5 }}
                    >
                      {'\u27F3'}
                    </button>
                  </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  {tokenisation.mode === 'stamp' && selectedImage && (
                    <button
                      className="ghost spread-nav"
        ...
        {tokenisation.mode === 'mint' ? (
          <MintPanel
            ...
            onAddImageFromUrl={mint.addImageFromUrl}
          />
        ) : tokenisation.mode === 'tokenise' ? (
          <TokenisePanel
      ...
      {tokenisation.mode !== 'watch' && tokenisation.mode !== 'mix' && tokenisation.mode !== 'cover' && tokenisation.mode !== 'titles' && tokenisation.mode !== 'export' && tokenisation.mode !== 'ticket' && (
        <PageStrip
          spreads={spreads}
          allImages={images}
          activeIndex={currentSpreadIndex}
