...
import type { MintDocument } from '../lib/types';
...
export default function MintCanvas({ doc, renderToCanvas, onSelectLayer, onUpdateConfig, selectedLayerId, showGrid, animatePreview }: Props) {
  ...
  // Wheel: zoom selected image layer, or pan canvas
  const handleWheel = useCallback((e: React.WheelEvent) => {
    // If an image layer is selected, scroll zooms the image
    const selectedLayer = selectedLayerId ? doc.layers.find(l => l.id === selectedLayerId) : null;
    if (selectedLayer && selectedLayer.type === 'image' && onUpdateConfig) {
      e.preventDefault();
      const currentScale = (selectedLayer.config as any).scale || 1;
      const delta = e.deltaY > 0 ? 0.95 : 1.05;
      const newScale = Math.max(0.1, Math.min(5, currentScale * delta));
      onUpdateConfig(selectedLayerId!, { scale: newScale });
      return;
    }
  ...
  }, [selectedLayerId, doc.layers, onUpdateConfig]);

  // Image layer drag state
  const isDraggingImage = useRef(false);
  const imageStartPos = useRef({ x: 0, y: 0 });
  const imageOriginConfig = useRef({ x: 0.5, y: 0.5 });

  // Pan with middle-click or Alt+drag; Image drag with primary click on selected image layer
  const handlePointerDown = useCallback((e: React.PointerEvent) => {
    ...
    } else if (e.button === 0 && selectedLayerId) {
      // Check if selected layer is an image — enable drag to reposition
      const layer = doc.layers.find(l => l.id === selectedLayerId);
      if (layer && layer.type === 'image') {
        e.preventDefault();
        isDraggingImage.current = true;
        imageStartPos.current = { x: e.clientX, y: e.clientY };
        imageOriginConfig.current = { x: (layer.config as any).x ?? 0.5, y: (layer.config as any).y ?? 0.5 };
        (e.target as HTMLElement).setPointerCapture(e.pointerId);
      }
    }
  }, [pan, selectedLayerId, doc.layers]);
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    } else if (isDraggingImage.current && selectedLayerId && onUpdateConfig) {
      // Convert pixel delta to normalized 0-1 offset
      const canvas = canvasRef.current;
      if (!canvas) return;
      const rect = canvas.getBoundingClientRect();
      const dx = (e.clientX - imageStartPos.current.x) / rect.width;
      const dy = (e.clientY - imageStartPos.current.y) / rect.height;
      onUpdateConfig(selectedLayerId, {
        x: Math.max(-1, Math.min(2, imageOriginConfig.current.x + dx)),
        y: Math.max(-1, Math.min(2, imageOriginConfig.current.y + dy)),
      });
    }
  }, [selectedLayerId, onUpdateConfig]);
